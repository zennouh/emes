...
function enrichTeamsData(array $teams)
{
    global $clubRepo;
...
function enrichPlayersData(array $players)
{
    global $teamRepo, $clubRepo;
...
function enrichMatchesData(array $matches)
{
    global $teamRepo, $tournamentRepo;
...
function displayEnrichedData(array $data, string $type)
{
    switch ($type) {
        case 'team':
            $enriched = enrichTeamsData($data);
            $headers = ['id', 'name', 'club_name', 'game', 'created_at'];
            break;

        case 'player':
            $enriched = enrichPlayersData($data);
            $headers = ['id', 'pseudo', 'team_name', 'club_name', 'role', 'salary', 'created_at'];
            break;

        case 'match':
            $enriched = enrichMatchesData($data);
